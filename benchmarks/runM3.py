...
def vectrixForecast(train: np.ndarray, horizon: int, period: int) -> np.ndarray:
    ...
    from vectrix.engine.baselines import RandomWalkDrift
    ...
    if effectivePeriod > 1 and n >= effectivePeriod * 2:
    ...
    foldSize = min(horizon, n - horizon - 3)
    if foldSize < 3:
        foldSize = 3

    cvTrain = train[:n - foldSize]
    cvTest = train[n - foldSize:]
