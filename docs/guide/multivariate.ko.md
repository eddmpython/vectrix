...
VAR 모델은 여러 시계열 간의 선형 상호의존성을 포착합니다. 각 변수는 자신의 시차값과 다른 모든 변수의 시차값의 선형 결합으로 모델링됩니다.

```python
from vectrix.engine.var import VARModel
import numpy as np

# Y shape: (T, k) — T 시점, k 변수
...
# predictions shape: (12, 3) — 12 스텝, 3 변수
```

### 자동 시차 선택

모델이 1부터 `maxLag`까지 모든 시차 차수를 테스트하고, 정보 기준에 따라 최적 차수를 선택합니다

```python
model = VARModel(maxLag=8, criterion="aic")   # Akaike IC (기본값)
model = VARModel(maxLag=8, criterion="bic")   # Bayesian IC (더 간결)
...
if result['pValue'] < 0.05:
    print("변수 0이 변수 1을 Granger-인과합니다")
```
...
| 정상 시리즈 | VAR |
| 비정상이지만 공적분 | VECM |
| 비정상, 공적분 없음 | 차분 데이터에 VAR |
| 불명확 | 둘 다 시도 후 비교 |

### 공적분 랭크

VECM은 공적분 랭크(독립 공적분 관계 수)를 자동 추정합니다

```python
model = VECMModel(maxLag=4, rank=None)  # 자동 감지
model = VECMModel(maxLag=4, rank=1)     # rank=1 강제
...
**API 레퍼런스:** [VAR/VECM API](../api/multivariate.md)
